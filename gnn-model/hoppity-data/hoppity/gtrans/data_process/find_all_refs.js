...
function visit(node, filename, ast){
	try{
		if(node.getKind() === ts.SyntaxKind.Identifier){
			let refs = [];
			try { refs = node.getDefinitionNodes(); }
			catch (e) { refs = []; }
			...
			refs.forEach(function(ref){
                ...
                console.log(`Error in file ${filename} --- ${e.stack}`);
		// console.log('AST', ast)
	}
}
...
try {
	...
	console.log('Error parsing ast', e.stack);
}
...
sourceFile.forEachDescendant(node => { 
	try { visit(node, src_file, ast) }
	catch(e) { console.log('Visit error', e.stack); }
});
// console.log(JSON.stringify(ref_map))
// console.log(`AST of file ${src_file}--- `, ast)
